...
for i in dominant_frequencies1:
           #plotting the input trace
           time_axis1 = np.linspace(0, len(i), len(i))
           plt.figure(figsize=(10, 6))
           plt.plot(time_axis1, i, color='black', linewidth=0.5)
...
for i in spectral_energy_distribution1:
           #plotting the input trace
           time_axis1 = np.linspace(0, len(i), len(i))
           plt.figure(figsize=(10, 6))
           plt.plot(time_axis1, i, color='black', linewidth=0.5)
...
for i,j in zip(XX,y):
       ...
       cross_corr = np.correlate(i, j, mode='full')
       # Calculate time lags corresponding to each cross-correlation value
       time_lags = np.arange(-len(i) + 1,len(i))
