...
denoised_trace1 = seismic_trace -medfilt(seismic_trace, kernel_size=window_size)
time_axis = np.linspace(0, len(denoised_trace1), len(denoised_trace1))
plt.figure(figsize=(10, 6))
...
seismic_trace = (merged_vector - np.min(merged_vector)) / (np.max(merged_vector) - np.min(merged_vector))
denoised_signal2 = seismic_trace-wiener(seismic_trace)
time_axis = np.linspace(0, len(merged_vector), len(merged_vector))
plt.figure(figsize=(10, 6))
...
denoised_trace3 = seismic_trace-gaussian_filter(seismic_trace, sigma=sigma)
time_axis = np.linspace(0, len(merged_vector), len(merged_vector))
plt.figure(figsize=(10, 6))
...
denoised_trace4 =seismic_trace-pywt.waverec(coeffs, wavelet)[:808101]
...
fk_denoised_trace =seismic_trace_2d- fk_filter(seismic_trace_2d)
...
def fk_filter(trace):
    ...
    denoised_trace = np.fft.ifft2(fk_data).real
    return denoised_trace
fk_denoised_trace = seismic_trace_2dd-fk_filter(seismic_trace_2dd)
time_axis = np.linspace(0, len(fk_denoised_trace), len(fk_denoised_trace))
plt.figure(figsize=(10, 6))
plt.plot(time_axis, fk_denoised_trace, color='black', linewidth=0.5)
plt.xlabel("Time (s)")
