...
for j in trace_files:
     ...
     time_axis1 = np.linspace(0, len(sum_vector), len(sum_vector))
     plt.figure(figsize=(10, 6))
     plt.plot(time_axis1, sum_vector, color='black', linewidth=0.5)
     plt.xlabel("Time (s)")
     ...
     sum_vector1 = [sum(elements) for elements in zip(*p2)]  
     #plotting the input trace
     time_axis2 = np.linspace(0, len(sum_vector1), len(sum_vector1))
     plt.figure(figsize=(10, 6))
     plt.plot(time_axis2, sum_vector1, color='black', linewidth=0.5)
     plt.xlabel("Time (s)")
     plt.ylabel("Amplitude")
     plt.title("Seismic filtered_merged Trace")
     # Save the plot in the folder
     ...
     # Merged vectors into a single vector    
     sum_vector2 = [sum(elements) for elements in zip(*p4)]
     #plotting the input trace
     time_axis4 = np.linspace(0, len(sum_vector2), len(sum_vector2))
     plt.figure(figsize=(10, 6))
     plt.plot(time_axis4, sum_vector2, color='black', linewidth=0.5)
     plt.xlabel("Time (s)")
     plt.ylabel("Amplitude")
     plt.title("Seismic filtered_merged Trace")
     # Save the plot in the folder
