...
# Loop through the trace files and visualize each processesed trace
...
for trace_file in trace_files[10:11]:
 if trace_file!='op.npy':
    ...
    seismic_trace = np.load(trace_path)
    #sigma=2.0
    #smoothed_data = gaussian_filter1d(seismic_trace, sigma)
    #(np.linespace)create an array of time values that correspond to each data point in the seismic trace.
    time_axis = np.linspace(0, len(seismic_trace), len(seismic_trace))
    plt.figure(figsize=(10, 6))
